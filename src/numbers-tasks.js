...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const productOfVectorsLengths =
    Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const angle = scalarProductOfVectors / productOfVectorsLengths;
...
function toNumber(value, def) {
  ...
  if (typeof convertedValue === 'undefined' || Number.isNaN(convertedValue)) {
    return def;
  }
...
function getSumOfDigits(num) {
  ...
  const convertedDigits = digits.map(Number);
  const sum = convertedDigits.reduce((elem, acc) => acc + elem, 0);
  return sum;
}
...
function getCountOfOddNumbers(number) {
  const num = number;
  let counter = 0;
  for (let i = 0; i <= Math.abs(num); i += 1) {
    if (i % 2 === 1) {
      counter += 1;
    }
  }
